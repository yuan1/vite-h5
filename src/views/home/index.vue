<template>
  ...
  <van-form @submit="onSubmit">
    <van-cell-group inset>
      <van-field v-model="username" name="用户名" label="用户名" placeholder="用户名" required
  ...
  <div class="check-list">
    <p v-for="s in showList" :key="s" :class="checkList.includes(s.key) ? 'active' : ''" @click="handleCheck(s)">{{
      s.key
    }}:{{ s.value }}</p>
  </div>
  ...
  <template v-else>
    <p v-if="getInterval('2022/08/31') > 0">{{ getInterval('2022/08/31') }}->d2</p>
    <p v-if="getInterval('2022/08/15') > 0">{{ getInterval('2022/08/15') }}->d3</p>
  </template>
</template>
